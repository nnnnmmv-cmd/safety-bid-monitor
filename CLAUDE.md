...
- 공고문 첨부 링크는 `hub_sync.build_spec_docs()` → 허브 `spec_docs`(`[{url,name}]`, 게시 순서 유지). 허브가 이름으로 공고문을 골라 알림 줄에 📎를 붙이므로 **이름 품질이 곧 기능** — `_clean_doc_name()`이 non-breaking space를 펴고(안산시), 링크 글자가 버튼 문구면(남양주시 '내려받기') URL의 `user_file_nm`에서 원본 파일명을 되찾는다. **bids엔 첨부 컬럼이 없어** 수집 시점에 `posting.attachments`를 넘기지 않으면 나중엔 detail 재조회 말곤 방법이 없다. 백필: `scripts/backfill_spec_docs.py`(파일 다운로드·LLM 없이 링크만 긁음)
