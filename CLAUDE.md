...
- 게시판 방문 기록은 `store.log_site_health()` → `bid_collection_health`(site_name·run_at·reason·posts_saved·posts_fetched). `_process_site`가 사이트당 1행씩 남기며 **새 글이 0건이어도 남긴다** — 흔적이 없으면 죽은 게시판과 원래 조용한 게시판을 구분할 수 없다. **`posts_fetched`는 `adapter.rows_seen`(날짜 필터 이전의 목록 행 수)이지 수집된 글 수가 아니다** — `fetch()`가 돌려주는 postings를 쓰면 옛 글만 있는 조용한 게시판도 0이 되어 파싱이 깨진 게시판과 구분이 안 된다. `posts_fetched=0`이 곧 '목록이 안 읽힘'. `site_name`은 `sites.name` 그대로(bids.site_name의 옛 표기와 다름). `reason`은 `_health_reason()`이 유한한 코드로 정규화한다(ok/timeout/http_error/ssl_error/connection_error/dns_error/error). **0행은 `no_match`(검색 결과 없음)와 `no_rows`(목록을 못 읽음)로 갈린다** — 사이트 설정이 아니라 **응답 HTML에 목록 표가 있는지**로 판단한다(`adapter.list_container_found`). 군포시 실측: 검색 결과 0건이어도 `table tbody`는 존재하고 그 안이 비어 있다. 표가 없으면 selector가 안 맞는 것이므로 `no_rows`=장애. `no_match`는 정상으로 읽되 몇 주씩 지속되면 게시판 URL이 엉뚱한 곳을 가리키게 바뀐 것일 수 있으니 한 번 열어볼 것(군포시-입찰공고: notAncmtSeCd=02에 '안전'이 든 글이 없어 0행, 안전점검 공고는 01 게시판이 잡고 있어 놓치는 건 없음) — 예외 원문을 그대로 넣으면 URL·타임아웃 초가 섞여 묶이지 않는다. 기록 실패는 삼키고 수집을 계속한다. 테이블 DDL: `scripts/bid_collection_health.sql`
...
- 공고문 첨부 링크는 `hub_sync.build_spec_docs()` → 허브 `spec_docs`(`[{url,name}]`, 게시 순서 유지). 허브가 이름으로 공고문을 골라 알림 줄에 📎를 붙이므로 **이름 품질이 곧 기능** — 기준은 **이름에 '공고'가 들어 있는가**다(확정 키워드는 공고문·입찰공고·공고서, 후보도 '공고'가 든 문서파일로 한정. 붙임이라도 '공고'가 있으면 뽑히고 없으면 애초에 후보가 아니다). 첨부명에 '공고'가 없는 게시판이 새로 붙으면 링크가 안 달리니 그때 확인할 것 — `_clean_doc_name()`이 non-breaking space를 펴고(안산시), 링크 글자가 버튼 문구면(남양주시 '내려받기') URL의 `user_file_nm`에서 원본 파일명을 되찾는다. **bids엔 첨부 컬럼이 없어** 수집 시점에 `posting.attachments`를 넘기지 않으면 나중엔 detail 재조회 말곤 방법이 없다. 백필: `scripts/backfill_spec_docs.py`(파일 다운로드·LLM 없이 링크만 긁음)
- 면허(`license_limit`)·참가지역(`region_limit`)은 **조달청 목록·보조 API에 안 실리고 첨부 공고문 안에만 있다**(허브 실측 554건 중 면허 299건 누락). `summarizer.extract_bid_fields` 9필드에 포함 → `hub_sync._limit_cols()` → 허브. 형식은 조달청 보조 API와 동일: 면허 `' / '`, 지역 `'·'`. **지역은 시·군 단위 필수** — '경기도'만 쓰면 허브가 경기 전역으로 읽어 고양시 제한 건에 용인 본점을 통과시킨다. **NULL과 `''`는 뜻이 다르다** — NULL='아직 안 봤다'(허브가 나중에 재시도), `''`='봤는데 제한 없음'(재시도 안 함). `extract_bid_fields`는 타임아웃·파싱 실패에도 전 필드 빈 dict를 돌려주므로, `_limit_cols`가 **전 필드가 비면 실패로 보고 칸을 아예 안 보낸다**(안 그러면 안 읽은 공고가 '확인 완료'로 굳는다). 이미 값이 있는 칸은 `update_limits`가 `.is_(칸,"null")` 필터로 덮지 않는다(조달청 보조 API 값이 더 정확). 백필: `scripts/backfill_limits.py`
- 나라장터(`source='g2b'`) 행엔 `license_limit`·`region_limit`을 **쓰지 말 것** — 허브 collectBids가 조달청 보조 API로 채우는 칸이라 두 writer가 되면 값의 출처를 못 가린다. g2b는 크롤러 단독 소유인 `requirements`에만 넣는다(허브 `bid-filter.ts`가 폴백으로 읽음). **`requirements.licenses`는 원문 문장 그대로 둘 것** — 허브 `registryVerdict()`가 그 문장에서 '명부'·'등록'을 찾아 명부 판정을 한다('예산군 건축공사 안전점검 수행기관 등록명부에 등록된 업체'). 면허 이름만 추린 배열은 `requirements.license_names`에 따로 담는다(2026-08-19 신설). 정규화로 덮으면 '조달청 통합명부' 판정 4건이 죽는다 — 그건 발주기관명으로는 절대 안 나오는 유일 경로. `region`은 **자동 판정에 쓰지 않는다(2026-08-19 확정)** — 한 칸에 제한·가산점·명부가 섞여 들어와 규칙으로 못 가른다('사무소 소재지가 천안시인 경우 지역업체가산 3점'은 제한이 아니라 가산점, '경기도 안전점검수행기관 등록명부에 등록된 업체로 제한함'은 지역이 아니라 명부 요건). 허브 실측 44건에서 확인. '지역 확인' 배지 표시용이라 시·군 정확도를 위해 프롬프트를 조일 필요가 없다 — 조이면 없는 시·군을 지어내는 쪽이 더 위험하다. 배열 칸은 `_LIST_FIELDS`로 강제한다 — LLM이 `"['a','b']"` 문자열로 돌려주면 허브가 문자 하나씩 순회한다(실측 61자→61항목). 사이클 추출은 **마감 전 + relevance=likely**만 집어 마감된 건은 영영 안 돌아간다 → 백필: `scripts/backfill_g2b_requirements.py`(`--since`로 프롬프트 교정분만 재추출)
