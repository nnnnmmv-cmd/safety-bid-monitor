...
EMPTY_FIELDS: dict[str, str] = {
    ...
    "low_bid_rate": "",
    "winner_selection": "",
    # 아래 둘은 조달청 목록·보조 API에 안 실리고 첨부 공고문 안에만 있다 (허브 실측 554건 중 면허 299건 누락).
    # 빈 문자열 = "공고문을 읽었고 제한이 없었다". 아직 안 읽은 건 hub_sync가 키 자체를 안 보내 NULL로 남긴다.
    "license_limit": "",
    "region_limit": "",
}

SYSTEM_PROMPT: str = """당신은 한국 공공 건설공사 안전점검 입찰공고를 분석하는 전문가입니다.
주어진 공고 제목과 본문에서 아래 9개 필드를 추출하세요.
...
- license_limit: 참가에 요구되는 면허·등록 자격. 여러 개면 " / "로 이어 붙일 것.
  등록번호가 적혀 있으면 "이름/번호" 형태로, 없으면 이름만.
  예: "안전진단전문기관(건축분야)/1397 / 안전진단전문기관(종합분야)/4963"
  예: "안전진단전문기관(교량 및 터널분야)/1395 / 안전점검전문기관(토목 분야)/6208"
  예: "안전진단전문기관(종합분야)"
  자격 요구가 공고문에 없으면 빈 문자열.
- region_limit: 참가 가능한 본점 소재지 제한. 여러 곳이면 "·"로 이어 붙일 것.
  **반드시 시·군 단위까지 적을 것** — "경기도"만 쓰면 경기 전역으로 오해된다.
  광역만 적힌 게 아니라 시·군이 열거돼 있으면 그 시·군을 모두 "시·도 시·군" 형태로 쓸 것.
  예: "경기도 고양시·경기도 파주시·경기도 김포시"
  예: "경상남도 하동군·경상남도 진주시·경상남도 사천시·경상남도 남해군"
  제한이 없거나 전국이면 "전국".
  지역 제한 언급이 공고문에 아예 없으면 빈 문자열.

응답 형식 — 반드시 아래 JSON 한 줄로만 출력. 마크다운 코드블록·설명·인사 금지:
{"inspection_cost":"...","contractor":"...","scale":"...","bid_period":"...","evaluation_method":"...","low_bid_rate":"...","winner_selection":"...","license_limit":"...","region_limit":"..."}"""
